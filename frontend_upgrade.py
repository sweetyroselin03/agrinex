...
import React from 'react';
import { motion } from 'framer-motion';
import { BellIcon, ChartBarIcon, ArrowTrendingUpIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-6">
      <header className="flex justify-between items-center mb-8 pt-4">
        ...
        <Link to="/notifications" className="w-10 h-10 glass-card flex items-center justify-center relative">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="glass-card p-5 group block relative overflow-hidden">
             <div className="w-10 h-10 bg-accent/20 rounded-xl flex items-center justify-center mb-4 border border-accent/30">
...
export default function Community() {
  ...
  const handleLike = async (id: number) => {
    try {
      ...
      setPosts(posts.map(p => p.id === id ? { ...p, is_liked: res.data.liked, likes_count: res.data.likes_count } : p));
    } catch (e) {}
  };
  ...
  const handleSave = async (id: number) => {
    try {
      ...
      setPosts(posts.map(p => p.id === id ? { ...p, is_saved: res.data.saved } : p));
    } catch (e) {}
  };
...
export default function Chatbot() {
  ...
  const sendMsg = async (textToSend: string = input) => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === typingId ? { id: res.data.id, text: res.data.message, is_ai: true } : m));
    } catch (e) {
      setMessages(prev => prev.map(m => m.id === typingId ? { id: typingId, text: "Connection error.", is_ai: true } : m));
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full bg-bgMain overflow-y-auto pb-32">
      <div className="h-40 bg-gradient-to-br from-purple/40 via-cyan/20 to-bgMain relative">
         <div className="absolute top-8 right-6 p-2 glass-card rounded-full"><Cog6ToothIcon className="w-5 h-5 text-textMain" /></div>
      </div>
...
print("Enterprise frontend generator written.")
