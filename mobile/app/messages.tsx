...
export default function MessagesScreen() {
  ...
  const handleStartMessage = async (targetUserId: number) => {
          ...
          <ScrollView contentContainerStyle={styles.scrollList} keyboardShouldPersistTaps="handled">
            {searchQuery.trim().length > 0 ? (
                ...
                {isSearchingUsers ? (
                  ...
                  userSearchResults.map((u) => (
                    <TouchableOpacity
                      ...
                      onPress={() => handleStartMessage(u.id)}
                      activeOpacity={0.85}
                    >
                      <Image
                        source={{
                          uri:
                            u.profile_picture ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(u.full_name || u.username || 'Farmer')}&background=16A34A&color=fff`,
                        }}
                        style={styles.avatar}
                      />
                      <View style={styles.userInfo}>
                        <Text style={styles.userName}>{u.full_name || u.display_name}</Text>
                  ...
                  <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptyTitle}>No Matching Farmers</Text>
                    <Text style={styles.emptyDesc}>No user matching "{searchQuery}" was found.</Text>
                  </View>
                )}
              </View>
            ) : isLoadingConversations ? (
              ...
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyDesc}>Connect and start a direct conversation with active farmers in your network.</Text>

                {suggestedFarmers.length > 0 && (
                  <View style={{ width: '100%', marginTop: 20 }}>
                    <Text style={{ fontSize: 14, fontWeight: '700', color: '#334155', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                      Suggested Farmers
                    </Text>
                    {suggestedFarmers.map((farmer) => (
                      <TouchableOpacity
                        ...
                        onPress={() => handleStartMessage(farmer.id)}
                        activeOpacity={0.85}
                      >
                        <Image
                          source={{
                            uri:
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(farmer.full_name || 'Farmer')}&background=16A34A&color=fff`,
                          }}
                          style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: '#E2E8F0' }}
                        />
                        <View style={{ flex: 1, marginLeft: 12 }}>
                          <Text style={{ fontSize: 15, fontWeight: '700', color: '#0F172A' }}>
                            {farmer.full_name || farmer.display_name || 'Farmer'}
                          </Text>
                          <Text style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>
                            📍 {farmer.village || 'Agricultural Hub'}
                          </Text>
                        </View>
                        <TouchableOpacity
                          style={styles.messageActionBtn}
